...
images_directory = output_directory + "images_" + str(args.File)[:-7] + "/"
...
command = 'cp ../u0_pp.asc .'
os.system(command)
command = 'cp ../u0.geom .'
os.system(command)

#### Read into class
var = ut.read_ASC_PP(images_directory, str(args.File)[:-7])
...
if n == 0:
    ...
    for j in range(0, ff.Nx):
        ut.plot_Contour(x_directory, str(args.File[:-7]), ff.z, ff.y, ff.velocityField[i, j, :, :], format(ff.x[j], '.2f'), ":", ":", ff.Re, str(j), "-", "-", ff.velocityField[m, j, :, :], ff.velocityField[p, j, :, :], "z", "y", velName)
        print(j)
    ...
    for j in range(0, ff.Ny):
        ut.plot_Contour(y_directory, str(args.File[:-7]), ff.z, ff.x, ff.velocityField[i, :, j, :], ":", format(ff.y[j], '.2f'), ":", ff.Re, "-", str(j), "-", ff.velocityField[m, :, j, :], ff.velocityField[p, :, j, :], "z", "x", velName)
        print(j)
    ...
    for j in range(0, ff.Nz):
        ut.plot_Contour(z_directory, str(args.File[:-7]), ff.x, ff.y, ff.velocityField[i, :, :, j].T, ":", ":", format(ff.z[j], '.2f'), ff.Re, "-", "-", str(j), ff.velocityField[m, :, :, j].T, ff.velocityField[p, :, :, j].T, "x", "y", velName)
        print(j)
